...
const testConfigFullPath = path.join(
  DEFAULT_BASE_PATH,
...
function envVarBasedOnCIEnv(varNameRoot) {
  ...
  if (fs.existsSync(testConfigFullPath)) {
    ...
    console.log(
      `${configObj[combined] ? combined : varNameRoot} is being loaded from ${testConfigRelativePath}`
    );
...
export default function createTestEnvFile() {
  ...
  return appFromSA
    ...
    .then((customToken) => {
      ...
      console.log(`${DEFAULT_TEST_ENV_FILE_PATH} created successfully`);
