...
import { DEFAULT_BASE_PATH, DEFAULT_TEST_FOLDER_PATH } from './constants';

const testEnvFilePath = path.join(DEFAULT_BASE_PATH, 'cypress.env.json');
...
const serviceAccountPath = path.join(DEFAULT_BASE_PATH, 'serviceAccount.json');
...
(function runCreateTestEnv() {
  try {
    createTestConfig()
      ...
      .catch((err) => {
        /* eslint-disable no-console */
        console.error(`Error creating test env file: ${err.message || 'Error'}`);
        /* eslint-enable no-console */
        process.exit(1);
      });
  }
